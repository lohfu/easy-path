...
var qs = _interopRequireDefault(require("querystring"));
...
function routeFromLink(node) {
  ...
  const target = node.getAttribute('target'); // ignore links with targets and non-path URLs
...
function delegateLinkHandler(e) {
  ...
  do {
    if (String(t.nodeName).toUpperCase() === 'A' && t.getAttribute('href')) {
      if (e.button !== 0) return; // if link is handled by the router, prevent browser defaults
...
function store(data) {
  setUrl((0, _util.getCurrentUrl)(), Object.assign({}, window.history.state, data), 'replace');
}
/*
 * precedence
 *
...
class Router {
  constructor(options) {
    ...
    Object.assign(this, (0, _lodash.pick)(options, 'canRoute', 'context', 'realm', 'remember', 'root'));
    ...
    this.layers = []; // TODO where should this be? should it even be in the Router class?
    ...
    if (typeof addEventListener === 'function') {
      if (options.popstate) {
        ...
        addEventListener('popstate', () => {
          this.exec((0, _util.getCurrentUrl)(), true);
        });
      }
    ...
    return (this.context ? this.context(url) : Promise.resolve({
      ...
      const stack = this.layers.reduce((result, [path, fnc]) => {
        ...
        if (!path) {
          ...
          const params = (0, _util.match)(url, path);
  ...
  start(options = {}) {
    ...
    if (options.trigger) {
      ...
      return this.exec(options.url || (0, _util.getCurrentUrl)(), options.run);
    }
